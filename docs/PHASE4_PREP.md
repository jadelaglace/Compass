...
### 3.4 Phase 4 收尾：服务连接与实时有效分

- Skill 通过 `COMPASS_API_URL` 定位服务，默认 `http://localhost:8080`；服务启用认证时，Skill 必须将 `COMPASS_API_TOKEN` 作为 `Authorization: Bearer` 头透传。Skill 不扫描端口、不推断地址。
- `score.interest`、`strategy`、`consensus` 与 `score.composite` 是基础分。时间推移不得改写它们，不得产生 `Decay` 历史，也不得由后台定时任务写回 Markdown 或 SQLite。
- 查询时计算 `effective_composite = base_composite * freshness_factor`。`freshness_factor` 来自实体创建/内容更新时间和可选知识时效配置；默认 `evergreen`，因而既有笔记的 factor 恒为 `1.0`。
- 仅内容或非 score metadata 的外部修改可以推进 `content_updated_at`；Compass 的 score 写回、访问记录与建议写回不得把知识重新标记为新鲜。
- Feed、Top、Search、Graph、Agent context、关联候选的 composite 信号和 skill renderer 都使用实时有效分；响应同时返回基础分与 factor，便于解释与 Dataview 保持稳定基础分。
...
| T4.8 | Skill 服务连接合同 | T4.7 | 读取环境配置；不保存 secret | `COMPASS_API_URL` 可覆盖默认值；设置 token 时全部请求带 Bearer；缺失/错误凭证有可读错误 |
| T4.9 | 实时有效分与知识时效 | T4.1,T4.6 | 只读计算；不写 Vault/SQLite | 不启动 decay scheduler；基础分不随时间变化；所有排序与渲染使用同一有效分；evergreen 与时效笔记覆盖测试 |

执行顺序固定为 `T4.0 → T4.1` 与 `T4.7` 并行准备；`T4.2` 必须等待二者完成；随后进入 `(T4.3/T4.4/T4.5) → T4.6 → T4.8 → T4.9`。T4.7 未完成时不得合并任何新增写接口。
...
- 所有 Phase 4 建议相关写操作均由显式 accept 触发，且通过内容版本校验；T4.9 明确替代持久化衰减合同，score/access/create/watcher 之外的写入边界不变。
- 建议、推荐、周报有固定 fixture 和 Rust 单元/E2E 测试。
- skill renderer 能处理成功、空结果、过期和拒绝结果。
- Skill 可通过环境变量连接任意受支持服务地址，并在配置 token 时认证；实时有效分在所有读接口和渲染中一致，且不产生时间驱动的 Vault/SQLite 写入。
