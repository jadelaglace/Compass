...
| **v2.2** | 2026-04-27 | 恢复阉割功能：前端TS锁定（Vue3+TS）、飞书Bot交互架构（修正）、FAISS搜索、定时任务、备份策略、PWA离线；**MCP Server推后**（Skill已满足当前需求） |
...
> **MCP Server 推后，不列入 Phase 2/3 开发计划。**

**原因：**
- OpenClaw Skill 已承担 Agent 交互入口角色（通过 OpenClaw 内置的 Feishu 集成）
- 你主要通过飞书 + OpenClaw Agent 使用 Compass，MCP-native 客户端（Cursor / Claude Code）直接调用的场景当前不存在
- MCP 增加维护成本，Phase 2 应聚焦核心价值
...
总工时：231h | 建议周期：12-16 周 | **MCP Server：推后，有实际需求再评估**
```
