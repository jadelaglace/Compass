...
| 4 | 智能增强 | 按需 | 进行中（T4.8-T4.9） | 可解释建议/周报 + 可配置 Skill 连接 + 实时有效分 |
...
## 4. Phase 4 · 智能增强（T4.0-T4.7 已完成，T4.8-T4.9 收尾中）
...
| T4.8 | Skill 服务连接合同 | T4.7 | 进行中 | `COMPASS_API_URL` 覆盖默认地址；可选 Bearer token 透传；E2E 覆盖自定义地址与鉴权 |
| T4.9 | 实时有效分与知识时效 | T4.8 | 进行中 | 基础分不因时间改写；无定时衰减；所有查询/渲染使用实时 `effective_composite` |
...
Phase 4 的 T4.0-T4.7 已完成；T4.8-T4.9 正在作为收尾交付。skill 已覆盖标签候选、关联推荐、accept/reject 与周报，并通过 action → Rust API → Vault/SQLite → render 的本地 E2E。
